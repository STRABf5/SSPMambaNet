    ...
    def __call__(self, p, targets):  # predictions, targets, model
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                iou = bbox_iou(pbox.T, tbox[i], x1y1x2y2=False,mpdiou_hw=640, MPDIOU=True)
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            if n:
                ...
                iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False,mpdiou_hw=640, MPDIOU=True)
                lbox += (1.0 - iou).mean()  # iou loss
    ...
    def __call__(self, p, targets, imgs):  # predictions, targets, model   
        ...
        for i in range(self.nl):  # layer index, layer predictions
            ...
            if n:
                ...
                selected_tbox[:, :2] -= grid
                #iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False, CIoU=True)  # iou(prediction, target)
                iou =iou = bbox_iou(pbox.T, selected_tbox, x1y1x2y2=False,mpdiou_hw=640, MPDIOU=True)
                lbox += (1.0 - iou).mean()  # iou loss
